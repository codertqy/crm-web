<template>
  <Dialog
    ...
    top="8vh"
  >
    <ProductManage :isShowHeader="false" ref="productManageRef" />
    <template #footer>
      <slot name="footer">
...
const getProductData = () => {
  if (productManageRef.value.proTable.selectedListIds.length > 1) {
    ...
    const params = {
      ...
      sales: productManageRef.value.proTable.selectedList[0].sales
    }
    console.log(productManageRef.value.proTable)
    emit('getProductData', params)
    dialogVisible.value = false
  }
}
</script>
